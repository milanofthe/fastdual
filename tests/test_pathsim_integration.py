...
def dual_jac(func, x):
    ...
    """
    reset()
    seeds = seed_array(np.atleast_1d(x))
...
class TestPerformance:
    ...
    def test_10x10_system(self):
        ...
        J_num = num_jac(lambda x: np.array([float(v) for v in coupled(seed_array(x0)).flat], dtype=float).reshape(-1) if False else coupled(x), x0)
